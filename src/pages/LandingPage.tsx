...
import { Zap, MapPin, BatteryCharging, ChevronRight, Moon, Sun } from 'lucide-react';
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-['Inter'] selection:bg-blue-200 dark:selection:bg-blue-900 transition-colors duration-300">
      {/* Navbar */}
      <nav className="fixed w-full z-50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-800/50 transition-colors duration-300">
      ...
      {/* Hero Section */}
      <div className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden bg-parallax" style={{ backgroundImage: "radial-gradient(circle at center, rgba(14, 165, 233, 0.05) 0%, transparent 70%)" }}>
              ...
              <div className="rounded-xl overflow-hidden border border-slate-100 dark:border-slate-800 bg-slate-100 dark:bg-slate-800 aspect-video relative flex items-center justify-center">
                ...
                <img 
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.parentElement!.innerHTML = '<div class="text-slate-400 dark:text-slate-500 font-medium text-lg">Gelişmiş Harita Arayüzü</div>';
                  }}
                />
              </div>
            </div>
      ...
      {/* Features Section */}
      <div id="ozellikler" className="py-24 bg-white dark:bg-slate-950 transition-colors bg-parallax" style={{ backgroundImage: "linear-gradient(to bottom, transparent, rgba(14, 165, 233, 0.02))" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Neden ŞarjRota?</h2>
            <p className="mt-4 text-lg text-slate-600 dark:text-slate-400">Yolculuğunuzu planlarken ihtiyacınız olan her şey tek bir yerde.</p>
          </div>
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    <div className="p-8 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-slate-900/50 transition-all duration-300">
