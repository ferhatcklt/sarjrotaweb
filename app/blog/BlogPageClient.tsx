        ...
        {filteredPosts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPosts.map((post, i) => (
              ...
              >
                <Link href={`/blog/${post.slug}`} className="h-48 overflow-hidden relative block">
                  <img 
                    src={post.image} 
                    alt={post.title}
          ...
          <div className="text-center py-20 bg-white dark:bg-slate-900/50 rounded-3xl border border-dashed border-gray-300 dark:border-slate-700">
            ...
            <Link href="/blog" className="inline-flex bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-full font-medium transition-colors">
              Tüm Yazılara Dön
            </Link>
          </div>
        )}
